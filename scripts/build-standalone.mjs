...
const preloadedClassPacksOutput = resolve(rootDir, 'data/preloadedClassPacks.generated.ts');

const preloadedClassPacks = existsSync(classPacksDirectory)
  ? readdirSync(classPacksDirectory, { withFileTypes: true })
    ...
    .flatMap((entry) => {
      try {
        return [{
          source: entry.name,
          classPack: JSON.parse(readFileSync(resolve(classPacksDirectory, entry.name), 'utf8')),
        }];
      } catch (error) {
        console.warn(`Skipping invalid preloaded Class Pack ${entry.name}: ${error.message}`);
        return [];
      }
    })
  : [];
...
writeFileSync(
  preloadedClassPacksOutput,
  `// Generated by scripts/build-standalone.mjs. Do not edit by hand.\nexport const PRELOADED_CLASS_PACK_JSON: ReadonlyArray<{ source: string; classPack: unknown }> = ${JSON.stringify(preloadedClassPacks, null, 2)};\n`,
);
...
const sourceFiles = [
  ...
  'data/preloadedClassPacks.generated.ts',
  'data/preloadedClassPacks.ts',
  'utils/assetPreloader.ts',
  'utils/avatarAssetPreload.ts',
  ...
  'components/ClassPackSelectionScreen.tsx',
